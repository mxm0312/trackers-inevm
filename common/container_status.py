...
class ContainerStatus:
    def __init__(self, host_web):
        ...
        self.get_id()
        self.events = [
            "/events/{}/before_start",  # Вызывается как можно раньше после запуска контейнера и перед началом работы скриптов обработки данных
            ...
            "/events/{}/before_end",
        ]  # Вызывается после окончания обработки данных в контейнере и сохранением выходных файлов в папке OUT_DIR и перед остановкой (удалением) контейнера
    ...
    def get_containers_from_docker(self) -> dict:
        ...
        result = (
            subprocess.check_output(
                'docker ps --format "{{.ID}}: {{.Names}}" --no-trunc', shell=True
            )
            .decode("utf-8")
            .split("\n")
        )
    ...
    def post_status(self, status: int, data: dict = None):
        ...
        """
        return self.post(
            urljoin(self.host_web, self.events[status].format(self.full_id)), data
        )
